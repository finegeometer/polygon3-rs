...
impl ConvexPolygon {
    ...
    pub fn from_boundaries(boundaries: impl IntoIterator<Item = Line>) -> Option<Self> {
        ...
        let mut boundaries: Vec<_> = boundaries
            .into_iter()
            .filter(|l| l.is_infinity() != Some(Ordering::Greater))
            .collect();
        ...
        match out.len() {
            2 => {
                if opposite_overlapping(out[0], out[1]) {
                    return None;
                }
            }
            3 => {
            ...
            4 => {
                if opposite_overlapping(out[0], out[2]) || opposite_overlapping(out[1], out[3]) {
                    return None;
                }
            }
            _ => {}
...
impl ConvexPolygon {
    ...
    pub fn test(point: Point, boundaries: Vec<Line>) {
        let boundaries_contain_point = boundaries
            .iter()
            .map(|l| point.cmp_line(*l))
            .min()
            .unwrap_or(Ordering::Greater);
