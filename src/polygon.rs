...
use crate::utils::{Line, Point, UnorientedLine};
...
impl std::convert::TryFrom<ConvexPolygon> for Polygon {
    ...
    fn try_from(poly: ConvexPolygon) -> Result<Self, InfiniteRegionError> {
        ...
        if edges.is_empty()
            || crate::utils::pairs(&edges)
                .any(|(e1, e2)| e1.intersect(*e2).sign() != Ordering::Greater)
        {
...
impl std::fmt::Display for InfiniteRegionError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "Tried to convert an infinite ConvexPolygon to a Polygon."
        )
    }
}
