...
impl Polygon {
	...
	pub fn operation<'r>(polygons: impl IntoIterator<Item = &'r Self>, inside: fn(&bit_vec::BitVec) -> bool) -> Self {
...
impl Polygon {
	...
	pub fn test_difference(self, point: crate::utils::Point, other: Vec<Self>) {
		...
		let answer_1 = self.contains(point).min(other.iter().map(|poly| poly.contains(point)).max().unwrap_or(Ordering::Less).reverse());
